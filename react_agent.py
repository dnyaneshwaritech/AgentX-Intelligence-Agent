...
# ============================================================
# EVALUATION / ADVERSARIAL TEST MODE
# ============================================================
...
def research_agent(state):
    ...
    if FORCE_TOOL_FAILURE:
        print("❌ Simulated Research Tool Failure", flush=True)

        return {
            "errors": [
                "Research tool failed during adversarial evaluation"
            ],
            "failed_tools": [
                "research"
            ],
    ...
    # तुमचा existing research agent code इथून पुढे
# ============================================================
# ENVIRONMENT
# ============================================================
...
def call_llm(
    ...
    try:
        ...
        print(
            "🔄 LLM fallback response activated",
            flush=True
        )

        # Safe fallback so graph can continue
        return (
            "LLM analysis is temporarily unavailable. "
            "Use the available tool evidence only. "
            "Do not make unsupported conclusions. "
            "Report uncertainty and continue with "
            "fallback planning."
        )
...
def router_node(state: AgentState):
    ...
    # ========================================================
    # MAX REPLAN / LOOP PROTECTION
    # ========================================================

    if iteration >= 4:

        print(
            "🛑 Maximum replanning reached"
        )

        print(
            "➡️ ROUTE → analysis "
            "(safe completion with available evidence)"
        )
    ...
    for failure in failures:
        ...
        if (
            "research" in failure_text
            or "search_research" in failure_text
        ):

            if "research" not in failed_tools:
        ...
        if (
            "news" in failure_text
            or "search_news" in failure_text
        ):

            if "news" not in failed_tools:
        ...
        if (
            "competitor" in failure_text
            or "search_competitor" in failure_text
        ):

            if "competitor" not in failed_tools:
    ...
    if (
        ...
        if "research" in failed_tools:

            print(
                "⚠️ Research tool previously failed"
            )
            ...
            # Use remaining evidence instead of retrying
            if "news" not in failed_tools:
    ...
    # ========================================================
        ...
        if "competitor" in failed_tools:

            print(
                "⚠️ Competitor tool previously failed"
            )
...
# ========================================================
    ...
    tracer.decision(
        decision_name="execution_mode",
        decision_value=(
            "adversarial"
            if adversarial
            else "normal"
        ),
        reason="Execution mode selected from request"
    )
    ...
    try:
        ...
        tracer.decision(
            decision_name="graph_execution",
            decision_value="start",
            reason="Initial state prepared successfully"
        )
        ...
        tracer.metric(
            "graph_execution_latency_seconds",
            round(execution_latency, 4)
        )
        ...
        tracer.decision(
            ...
            reason="LangGraph execution completed successfully"
        )
        ...
        tracer.decision(
            decision_name="failure_handling",
            decision_value="safe_stop",
            reason="Graph execution raised an exception"
        )
